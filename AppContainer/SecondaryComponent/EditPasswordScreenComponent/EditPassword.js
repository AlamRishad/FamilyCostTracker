...
import { updateSecondaryPassword } from "../../API/login";
const EditPassword = ({ route }) => {
  const { userId, familyMemberId, username } = route.params;
  console.log(userId, familyMemberId);
  ...
  const handleForgotPasswordPress = async () => {
    ...
    const result = await updateSecondaryPassword(
      userId,
      familyMemberId,
      oldpassword,
      newpassword
    ...
    if (result.success) {
      ...
      navigation.navigate("SecondaryProfile", { userId: userId });
    } else {
