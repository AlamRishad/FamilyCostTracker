...
const PeriodicityDetails = ({ route }) => {
  ...
  console.log(
    userId + "PeriodicityDetails" + username + "kire" + familyMemberId
  );
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchExpenses = async () => {
        try {
          const fetchedData = await fetchDailyExpensesByMember(
            userId,
            familyMemberId
          );
          let processedData;
  ...
  fetchFamilyMemberDetails(userId, username).then((result) => {
    if (result.success) {
